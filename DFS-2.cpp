...
void DFS_worker(long int edge_num)
{
	while (!ready)
		Sleep(1);
...
int main(int argc, char **argv)
{
	...
	t[1] = thread(&DFS_worker, gr->edge_count);    // activate the worker first

	start = true;
	syn_start.notify_all();
	t[0] = thread(&DFS_master, g, gr->edge_count);  //start up master thread
